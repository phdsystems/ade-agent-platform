...
class InMemoryMemoryProviderTest {
    ...
    @Mock private EmbeddingsProvider embeddingsProvider;
    ...
    void shouldStoreMemoryEntry() {
        ...
        MemoryEntry entry =
                new MemoryEntry(
                        null, "Test memory content", Map.of("type", "test"), Instant.now(), 0.8);

        // Mock embedding response
        when(embeddingsProvider.embed(anyString()))
                .thenReturn(Mono.just(new Embedding(List.of(0.1f, 0.2f, 0.3f), "test-model", 3)));
    ...
    void shouldRetrieveMemoryById() {
        // Given
        MemoryEntry entry =
                new MemoryEntry("test-id", "Test content", Map.of(), Instant.now(), 0.5);

        when(embeddingsProvider.embed(anyString()))
                .thenReturn(Mono.just(new Embedding(List.of(0.1f), "test", 1)));
    ...
    void shouldReturnEmptyForNonExistentId() {
        // When & Then
        StepVerifier.create(provider.retrieve("non-existent")).verifyComplete();
    }
    ...
    void shouldDeleteMemoryEntry() {
        // Given
        MemoryEntry entry =
                new MemoryEntry("delete-test", "Content to delete", Map.of(), Instant.now(), 0.5);
        ...
        // Verify it's gone
        StepVerifier.create(provider.retrieve("delete-test")).verifyComplete();
    }
    ...
    void shouldSearchByVectorSimilarity() {
        // Given
        when(embeddingsProvider.embed("document 1"))
                .thenReturn(Mono.just(new Embedding(List.of(1.0f, 0.0f), "test", 2)));
        when(embeddingsProvider.embed("document 2"))
                .thenReturn(Mono.just(new Embedding(List.of(0.8f, 0.2f), "test", 2)));
        when(embeddingsProvider.embed("query"))
                .thenReturn(Mono.just(new Embedding(List.of(0.9f, 0.1f), "test", 2)));
    ...
    void shouldFilterByMetadata() {
        // Given
        when(embeddingsProvider.embed(anyString()))
                .thenReturn(Mono.just(new Embedding(List.of(0.5f, 0.5f), "test", 2)));

        MemoryEntry entry1 =
                new MemoryEntry("1", "content 1", Map.of("category", "A"), Instant.now(), 0.5);
        MemoryEntry entry2 =
                new MemoryEntry("2", "content 2", Map.of("category", "B"), Instant.now(), 0.5);
    ...
    void shouldClearAllMemories() {
        ...
        for (int i = 0; i < 3; i++) {
            MemoryEntry entry =
                    new MemoryEntry("id" + i, "content " + i, Map.of(), Instant.now(), 0.5);
            provider.store(entry).block();
        }
        ...
        // Verify all are gone
        StepVerifier.create(provider.retrieve("id0")).verifyComplete();
    }
